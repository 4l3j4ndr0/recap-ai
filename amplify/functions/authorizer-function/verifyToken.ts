...
const getPemEncodedPublicKeysImplementation = async (jwksUrl: any) => {
...
const getPemEncodedPublicKeys = (region: string, userPoolId: string) => {
  ...
  if (!getPemEncodedPublicKeysPromisesCache.has(jwksUrl)) {
    ...
    getPemEncodedPublicKeysPromisesCache.set(
      jwksUrl,
      getPemEncodedPublicKeysImplementation(jwksUrl)
    );
  }
...
export const verifyToken = async (
  ...
  const pemEncodedPublicKeys = await getPemEncodedPublicKeys(
    region,
    userPoolId
  );
